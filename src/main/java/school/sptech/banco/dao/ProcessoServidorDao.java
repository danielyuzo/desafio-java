...
public class ProcessoServidorDao {
    public static ProcessoServidor buscarProcessoPorNomeEDadosServidor(Integer fkDadosServidor, String nome) {
        List<ProcessoServidor> listaProcessos = DatabaseUtils.CONEXOES[1].getConexaoDoBanco().query("""
            SELECT * FROM Processos WHERE fkDadosServidor = ? AND nomeProcesso = ?
            """, new ProcessoServidorRowMapper(), fkDadosServidor, nome);
//        List<ProcessoServidor> listaProcessos = DatabaseUtils.CONEXOES[0].getConexaoDoBanco().query("""
//            SELECT * FROM Processos WHERE fkDadosServidor = ? AND nomeProcesso = ?
//            """,new ProcessoServidorRowMapper(), fkDadosServidor, nome);
    ...
    public static void inserirProcesso(ProcessoServidor processo) {
        DatabaseUtils.CONEXOES[1].getConexaoDoBanco().update("""
            INSERT INTO Processos (nomeProcesso, fkDadosServidor) VALUES (?, ?)
